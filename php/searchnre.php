...
//Words
$k = (isset($_POST['search'])) ? $_POST['search'] : exit;
...
$cl->SetLimits((int)$offset,(int)$limit,200,0);
	...
	if($sort == 'desc')
	    $cl->SetSortMode(SPH_SORT_ATTR_DESC, $by); //New to old

	else
	    $cl->SetSortMode(SPH_SORT_ATTR_ASC, $by); //Old to new
	...
	<section class="container-link">
		<div class="rightInfo">
			...
			<span class="ninTopic">
				...
				<a id="go-'.$search_arr["to_id"].'" href="topic-'.$search_arr["to_id"].'" class="shref viewtopic"> Accéder au topic</a>
			</span>' : '').'
			<span class="timestamp">Crée à '.$timestamp.'</span>
			<a href="topic-'.$search_arr['to_id'].'-'.$search_arr['to_num_replies'].'" class="shref"> Dernier message à '.$timelast.'</a>
		</div>
		<div class="leftInfo">
	...
	<article class="container-body">'.$purifier->purify(bbcode_to_html(nl2br($search_arr["po_message"]))).'</article>
	';
	}
?>